...
def compute_sky_score_map(img_rgb):
    """
    Карта "похоже на небо" (0..1) по пикселям:
      - низкая насыщенность
      - высокая яркость
      - низкая текстура
    """
...
def find_horizon_dp(img_rgb, K=15, t=0.10, alpha=10,
                    ...
                    w_smooth=0.15):
    """
    Возвращает ys (длина W) и диагностические карты.
    """
    H, W, _ = img_rgb.shape
...
def choose_crop_y_conservative(img_rgb, ys, hazard_top=None):
    """
    Для пожара: режем очень аккуратно.
    1) Если линия слишком низко -> не режем.
    2) Если hazard_top высоко (есть дым/огонь) -> не режем выше него.
    """
    H, W, _ = img_rgb.shape
...
def _ann_to_binary_mask(ann, h, w):
    """
    Возвращает uint8 mask (h,w) из ann['segmentation']
    Поддерживает:
      - polygon: list[list[float]]
      - RLE: dict with 'counts'/'size'
      - uncompressed RLE (rare): dict with 'counts' as list
    """
...
def build_fire_smoke_masks(img_meta, anns, cats_map):
    """
    cats_map: {cat_id: cat_name} или {cat_id: {...}} — главное, чтобы можно было понять имя класса
    Возвращает:
      fire_mask, smoke_mask : uint8 (H,W) 0/1
    """
...
def _process_image_for_crop(img_path):
    """Helper function to load image and compute crop_y."""
...
def laws_maps_gpu(
    ...
    """
    Возвращает (147,H,W) float32 на GPU:
      - 49 масок Laws на каждый RGB канал
      - энергия = mean(resp^2) в окне window×window (BORDER_REFLECT)
      - опционально log1p для сжатия диапазона (полезно перед float16)
    """
...
def stats_maps_gpu(img_rgb_u8, img_hls_u8, img_ycbcr_u8, window=25, device="cuda", dtype=torch.float32, eps=1e-12):
    """
    Возвращает 35 карт статистических признаков: (35,H,W) на GPU.
    """
    pad = window // 2
...
def glcm_batch_bincount(q, dx, dy, levels):
    """
    q: (B,K,K) int64 or int32 in [0..levels-1]
    dx,dy: offsets
    returns C: (B,levels,levels) float32 on GPU
    """
    B, K, _ = q.shape
...
def haralick96_patches_gpu(hsl_u8_patches, levels=32, neighbor_size=5):
    """
    hsl_u8_patches: (B,3,25,25) uint8, каналы: H,S,L (0..255) как у твоего rgb_to_hsl_255.
    Возвращает: (B,96) float32 on GPU.
    """
...
def features_B_278_gpu_for_image_stride(
    ...
    """
    img_rgb_u8: (H,W,3) uint8 (уже crop/resize применены)
    stride: шаг центров (train/valid можно 4, test 1)

    returns:
      X278: (N,278) float16 numpy (CPU)
      cy,cx: (N,) int32 numpy (CPU) — координаты центров в текущем "resize-space"
    """
    assert img_rgb_u8.dtype == np.uint8
    H, W, _ = img_rgb_u8.shape
    r = window // 2
...
def build_h5_cache_B_split_gpu(
    ...
    """
    Builds ONE H5 file for a selected split.
    Adds:
      - meta/sample_start, meta/sample_count (row ranges per image)
      - robust shape/bounds checks
      - non-finite (NaN/Inf) checks
    """
...
def check_h5(
    ...
    """
    Validates structural correctness of saved H5:
      - dataset existence + shapes
      - all sample datasets have same length
      - img_idx in range
      - cy/cx non-negative
      - optional: scan X for NaN/Inf in chunks
      - optional: check meta/sample_start & meta/sample_count match samples/img_idx grouping
...
class H5InferDatasetFast(Dataset):
    """
    Для инференса: возвращает (x, img_idx, cy, cx).
    """
...
class SParabolaAct(nn.Module):
    """
    y = beta + sign(s)*sqrt(2*p*|s|)  (стабильная версия с eps)
    """
...
def pr_from_outputs(out, y_pm1, thr_out=0.0):
    """
    out: (N,1) или (N,) - выход последнего слоя (linear), НЕ sigmoid
    y_pm1: (N,1) или (N,) в {-1,+1}
    thr_out: порог по выходу сети
    """
...
def train_fastmlp_mae_fixed_thr(
    ...
    """
    Xtr, Xva уже на GPU (как ты делаешь).
    ytr_pm1, yva_pm1 должны быть {-1,+1} (uint8/float — не важно, приведём).
    """
    # y: (-1/+1) float (N,1)
    def prep_y_pm1(y):
...
# ------------------ One run (train cfg -> pick thr_out on val) ------------------
...
def make_thr_grid_from_outputs(
    ...
    """
    out_va: (N,1) или (N,) выходы модели на val (без sigmoid)
    Делает grid на [mean-k*std, mean+k*std], + опционально 0
    """
    if out_va.ndim == 2:
        out_va = out_va[:, 0]
...
def make_thr_grid_two_stage(out_va, n_wide=81, n_dense=81, dense_half=0.5, k=3.0):
    ...
    return grid
# =========================
# CELL 1: helpers (FIX SHAPES)
# =========================
def _as_col(y: torch.Tensor) -> torch.Tensor:
    """Make y shape (N,1) without copying if possible."""
    if y.ndim == 1:
        return y.unsqueeze(1)
    if y.ndim == 2 and y.shape[1] == 1:
        return y
    # fallback: flatten to (N,1)
...
def metrics_pm1_from_out(out, y_pm1, thr_out=0.0):
    """
    out:   (N,1) or (N,)
    y_pm1: (N,1) or (N,) in {-1,+1}
    pred = out >= thr_out => +1 else -1
    """
...
def pick_best_thr_out_from_out(
    ...
    """
    out_va: (N,1) or (N,) float
    yva_pm1: (N,1) or (N,) in {-1,+1}
    thr_grid: list of floats (thresholds on out)
    """
...
def grid_search_ins3_pbeta_thr(
    ...
    for p, beta in product(p_grid, beta_grid):
        ...
        # adaptive thr_grid around mean±4std (быстрее и точнее)
...
# ----------------------------
# binary metrics for y in {-1,+1} from out >= thr_out
# ----------------------------
@torch.no_grad()
...
def prf_from_cm_3class(cm):
    # returns per-class + micro/macro (over fire+smoke by default)
    out = {}
    ...
    # micro over smoke+fire (ignore non)
    cls_set = [1,2]
...
# ----------------------------
# image-level ratios and detection @ thresholds
# ----------------------------
def image_level_stats_from_offsets(
    h5, y3_pred_all, valid_mask_all=None, thresholds=(0.02,0.05,0.07,0.10)
):
    """
    y3_pred_all: (N,) labels {0,1,2} per sampled point (center grid)
    uses meta/sample_start & meta/sample_count to compute per-image ratios on sampled points.
    valid_mask_all: optional bool (N,) if you want to exclude points (usually None).
    """
    starts = h5["meta/sample_start"][:]
    counts = h5["meta/sample_count"][:]
    n_imgs = len(starts)
...
def hazard_metrics_mean_per_image(
    ...
    """
    Mean (macro) precision/recall over images:
      - compute P_i, R_i for each image (on sampled points)
      - then average across images (ignoring empty images)
    """
    if y_fire_pm1.ndim == 2: y_fire_pm1 = y_fire_pm1[:,0]
    if y_smoke_pm1.ndim == 2: y_smoke_pm1 = y_smoke_pm1[:,0]
...
def eval_one_model_on_test(
    ...
    model,
    Xte_n,
    y_task_pm1,             # y_smoke or y_fire in {-1,+1}
    thr_out: float,
    bs: int = 32768,
):
    # speed + inference
...
def eval_3class_from_two_outputs(
    ...
    # enforce mutual exclusion: smoke выключаем там, где fire==1
    smoke_pos = smoke_pos & (~fire_pos)

    # true 3-class from GT (fire priority)
    ...
    # image-level ratios via offsets
...
def save_outs_h5(path, **outs):
    """
    outs: named tensors, each (N,1) or (N,)
    saves float16 to reduce size. also stores N.
    """
    os.makedirs(os.path.dirname(path), exist_ok=True)
...
def eval_one_model_on_test_full(
    ...
    """
    Возвращает:
      rep: плоский dict со всеми метриками (глобальные + локальные(mean-per-image))
      out: (N,1) выход модели
    """
    if prefix is None:
    ...
    # 4) собрать всё в один плоский dict
    rep = {
        "name": name,
        "thr_out": float(thr_out),
    ...
    # local mean-per-image
    for k, v in rep_l.items():
        rep[f"mean_{k}"] = v

    return rep, out
# ============================================================
# 0) helpers (safe shapes + offsets)
# ============================================================

def _as_1d_pm1(y_pm1: torch.Tensor) -> torch.Tensor:
    """y in {-1,+1}, returns (N,) float on same device."""
    if y_pm1.ndim == 2:
        y_pm1 = y_pm1[:, 0]
    return y_pm1.float()

def _as_1d_out(out: torch.Tensor) -> torch.Tensor:
    """model out (N,1) or (N,) -> (N,) float"""
    if out.ndim == 2:
        out = out[:, 0]
    return out.float()

def _read_offsets(h5_path: str):
    """Read per-image sample ranges from H5."""
    with h5py.File(h5_path, "r") as h5:
...
# ============================================================
# 1) chunked inference (GPU-friendly)
# ============================================================
...
def infer_out_chunked(model, X, bs=32768, sync_cuda=True):
    """
    Returns:
      out: (N,1) float32 (on same device as X)
      elapsed_s: inference time in seconds (GPU-synced if sync_cuda=True)
      pix_per_s: N / elapsed_s
    """
    model.eval()
    device = X.device
    N = X.shape[0]
...
# ============================================================
# 2) binary metrics (global) for y in {-1,+1} using out >= thr_out
# ============================================================

@torch.no_grad()
def binary_metrics_pm1_from_out(out, y_pm1, thr_out=0.0):
    """
    Global binary metrics over ALL samples.
    pred_pos = out >= thr_out
    true_pos = y_pm1 > 0
    """
    out1 = _as_1d_out(out)
    y1  = _as_1d_pm1(y_pm1)
...
# ============================================================
# 3) binary metrics (mean per image) + image-level detection rates
# ============================================================
...
def binary_metrics_mean_per_image_from_offsets(
    ...
    """
    Computes:
      - mean(P), mean(R), mean(F1) across images (only images with c>0)
      - per-image predicted positive ratio and true positive ratio
      - image-level "found" rates @ thresholds:
          * recall_img@t : among GT-positive images (true_ratio>0), fraction with pred_ratio>=t
          * fpr_img@t    : among GT-negative images (true_ratio==0), fraction with pred_ratio>=t
          * det_img@t    : among all images, fraction with pred_ratio>=t
    Notes:
      - thresholds are fractions of pixels/points in that image (since test stride=1, it's real pixels).
    """
    if y_pm1_all.ndim == 2:
        y_pm1_all = y_pm1_all[:, 0]
    ...
    # image-level detection counters
    thr_list = list(thresholds)
...
# ============================================================
# 4) FULL EVAL for ONE model on test (global + mean-per-image + speed) + optional save out
# ============================================================
...
def eval_one_model_on_test_full(
    ...
    """
    Returns:
      rep: flat dict with ALL reasonable metrics:
        - global_* : global (all pixels)
        - mean_*   : mean-per-image + image-level detection rates @ thresholds
        - speed_*  : inference speed
      out: (N,1) model output (on GPU)
    Also can save outputs to disk so you don't re-run inference later.

    IMPORTANT:
      - y_pm1 expected in {-1,+1} (shape (N,) or (N,1))
      - out threshold is on raw out (NOT sigmoid)
    """
    if prefix is None:
        prefix = name.replace(" ", "_").replace("(", "").replace(")", "").replace("/", "_")
    ...
    return rep, out  # keep out on GPU for immediate downstream (committees/3-class), if needed
# ============================================================
# 0) Load saved outs (no inference)
# ============================================================

def load_saved_out_pt(path: str, map_location="cpu"):
    """
    Expected format (как мы сохраняли ранее):
      {"name":..., "thr_out":..., "out_f16": Tensor(N,), "n_samples":...}
    """
...
# ============================================================
# 1) Metrics from bool prediction (global)
# ============================================================
...
# ============================================================
# 2) Mean-per-image + image-level detection stats (smoke ratio)
# ============================================================
...
def mean_per_image_and_detection(
    ...
    """
    Returns:
      mean P/R/F1 across images,
      and image-level det/recall_img/fpr_img @ thresholds
    """
    if y_pm1_all.ndim == 2:
        y_pm1_all = y_pm1_all[:, 0]
    y_pm1_all = y_pm1_all.float()
...
def committee_pred_smoke(
    ...
    modes:
      - "voting_12":         C1 (INS1+INS2 voting)      => OR (эквивалент majority для 2)
      - "confirm_12":        C2 (INS1+INS2 confirmation)=> AND
      - "voting_123":        C3/C4 voting (3 models)    => majority (>=2 of 3)
      - "voting12_confirm3": C5/C6                      => (pred1 OR pred2) AND pred3
    """
    mode = mode.lower()
    if mode == "voting_12":
        return (pred1 | pred2)
...
def committee_vote_fraction(preds: list[torch.Tensor]):
    """Optional: per-pixel confidence proxy = fraction of voters that predicted smoke."""
    k = len(preds)
...
# ============================================================
# 4) Evaluate ONE committee + save predictions
# ============================================================
...
def eval_one_smoke_committee(
    ...
    Returns flat dict with:
      global_* metrics,
      mean_* metrics,
      plus optional saved paths.
    Also saves:
      - pred_u8: uint8 (N,) 0/1
      - vote_frac_f16 (optional): float16 (N,) in [0,1]
    """
    # committee prediction
    pred_comm = committee_pred_smoke(pred_ins1, pred_ins2, pred_ins3, mode=mode)

    # vote strength (useful for analysis / “уверенность”)
...
# ============================================================
# 5) MAIN: build smoke committees C1..C6 from saved outs
# ============================================================
...
def run_smoke_committees_from_saved_outs(
    ...
    out_ins3A_path: str,
    out_ins3B_path: str,
    # thresholds on raw out (YOU fill)
    thr_ins1: float,
    thr_ins2: float,
    thr_ins3A: float,
    thr_ins3B: float,
    ...
    return df
# ============================================================
# FIRE committees (F1/F2) from saved outs
# Requires already-defined:
#   - load_saved_out_pt
#   - eval_one_smoke_committee (универсальна для бинарного класса)
#   - committee_pred_smoke (логика voting/confirmation)
#   - global_metrics_from_pred_bool, mean_per_image_and_detection, etc.
# ============================================================
...
def run_fire_committees_from_saved_outs(
    ...
    rep_f1 = eval_one_smoke_committee(
        ...
        h5_test_path=h5_test_path,
        y_smoke_pm1=y_fire_pm1,         # тут просто y_fire
        pred_ins1=pred4,                # переиспользуем поля
        pred_ins2=pred5,
        pred_ins3=None,
        mode="voting_12",
        thresholds=thresholds_img,
...
def image_detection_rate_from_offsets(
    ...
    pred_pos_all: torch.Tensor,         # bool (N,)
    true_pos_all: torch.Tensor,         # bool (N,)
    thresholds=(0.02, 0.05, 0.07, 0.10) # fraction of predicted positives in image
):
    """
    "процент найденных пожаров" = доля изображений, где pred_ratio >= t
    + можно ещё посчитать recall@t на уровне изображений (если нужно).
    """
    thr_list = list(thresholds)
    ...
    for thr in thr_list:
        ...
        # image-level recall only among "true>=thr" images (если true>=thr мало, может быть шумно)
...
def smoke_committee_pred(mode: str, p1: torch.Tensor, p2: torch.Tensor, p3b: torch.Tensor | None):
    ...
    mode:
      - "v12"      : INS1 OR INS2
      - "c12"      : INS1 AND INS2
      - "v12_v3"   : (INS1 OR INS2 OR INS3B)   [voting 3]
      - "v12_c3"   : (INS1 OR INS2) AND INS3B  [voting then confirmation by INS3B]
    """
    if mode == "v12":
        return p1 | p2
    if mode == "c12":
...
def run_K_committees(
    ...
    # ---- committee definitions (your list) ----
    committees = [
        # minimal / smoke+fire
        ...
        # fire voting (INS4+INS5) + smoke minimal
    ...
    # Precompute y_true3 once
    y_true3 = make_3class_pred(y_fi, y_sm)

    for kid, desc, cfg in committees:
        # select which preds are present (so K1/K2 work as intended)
        ...
        if cfg["use_3"] is False:
            # only based on 1/2; still use sm mode v12/c12
        ...
        # 3-class pred (fire priority)
        y_pred3 = make_3class_pred(pred_fi, pred_sm)

        # -------- metrics: smoke/fire/hazard (binary) --------
        ...
        # -------- save predictions (so no recompute later) --------
        # store compact uint8 masks per-sample: 0/1 for smoke/fire/hazard and 0/1/2 for 3-class
        save_payload = {
            "id": kid,
            "desc": desc,
...
def mean_per_image_prf_3class_from_offsets(y_true3, y_pred3, starts, counts):
    # считаем per-image micro P/R/F1 по fire+smoke и усредняем по изображениям
    P_list, R_list, F1_list = [], [], []
    n_imgs = len(starts)
    for i in range(n_imgs):
        s = int(starts[i]); c = int(counts[i])
...
def found_fire_percent_by_gt_recall_thresholds(
    ...
    """
    Считаем по изображениям:
      recall_img = TP_hazard / GT_hazard (только если GT_hazard>0)
    found@t = доля изображений с GT_hazard>0, где recall_img >= t
    """
    thr_list = list(thresholds)
...
# ----------------------------
# committee definitions (editable)
# ----------------------------
def define_committees():
    """
    Каждый комитет: name + rule(out_dict, thr_dict)->(smoke_pos, fire_pos)
    out_dict keys: "ins1","ins2","ins3A","ins3B","ins4","ins5" (выходы (N,1) на GPU)
    thr_dict keys: соответствующие thr_out
    """
    C = []
...
def run_committees_and_save(
    ...
    # H5 file with committee predictions
    with h5py.File(out_save_path, "w") as h5:
        ...
        # (optional) store y_true3 for reproducibility (tiny: int8)
        ...
        for c in committees:
            ...
            # build outputs dict for this committee
...
def eval_committees_from_saved(
    ...
    with h5py.File(saved_committees_h5, "r") as h5:
        ...
        for key in comm_keys:
            ...
            # ---- hazard(2-class) global ----
            repH = hazard_metrics_from_bool(pred_h, true_h)

            # ---- mean-per-image (локальные) ----
            ...
            # ---- found fires by GT hazard recall thresholds ----
            ...
            # also store used thresholds (attrs)
...
def pos_from_out(out, thr_out: float):
    """out: (N,1) or (N,) float; return bool (N,)"""
    if out.ndim == 2: out = out[:, 0]
...
def majority_vote(pos_list, k=None):
    """
    pos_list: list[bool tensor (N,)]
    k: minimum votes to accept (default = ceil(n/2))
    """
...
def weighted_vote(pos_list, weights, thr_weight=None):
    """
    weights: list[float] same length as pos_list
    thr_weight: accept if sum(weights_i * vote_i) >= thr_weight
      default = 0.5 * sum(weights)
    """
...
def define_committees_voting():
    """
    outs keys: ins1 ins2 ins3A ins3B ins4 ins5
    thrs keys: same
    returns list of dicts with: name, uses, rule
    rule(outs, thrs) -> (smoke_pos, fire_pos)
    """
    C = []
    ...
    return C
# ВАЖНО: эти функции у тебя уже есть из прошлого шага:
# - run_committees_and_save(...)
# - eval_committees_from_saved(...)
# Мы добавим новый runner, который использует define_committees_voting()
...
def run_voting_committees_and_save(
    ...
    y_fire_pm1: torch.Tensor, # Added y_fire_pm1
    y_smoke_pm1: torch.Tensor, # Added y_smoke_pm1
):
    os.makedirs(os.path.dirname(out_save_path), exist_ok=True)
...
def define_committees_voting():
    ...
    # =========================================================
    # Идея:
    #   base model даёт кандидатов (где потенциально smoke/fire),
    #   confirmer подтверждает их. Это снижает FP.
    #
    # Smoke confirmation:
    #   вариант 1: (ins1 OR ins2) CONFIRMED BY ins3B
    #   вариант 2: ins2 CONFIRMED BY ins3B (если ins2 сильнее / меньше FP)
    #
    # Fire confirmation:
    #   ins4 CONFIRMED BY ins5  (или наоборот)
    # =========================================================
...
def make_y3_from_gt_pointwise(y_fire01: torch.Tensor, y_smoke01: torch.Tensor) -> torch.Tensor:
    """
    y_fire01, y_smoke01: (N,) or (N,1) in {0,1} (uint8/float/etc)
    returns y3: (N,) int64 in {0,1,2} with fire priority
    """
...
def _infer_grid_shape_from_centers(cy_np: np.ndarray, cx_np: np.ndarray, Hr: int, Wr: int, R: int, stride: int):
    """
    Пытаемся восстановить (ny,nx) сетки центров.
    Обычно центры идут по y-major: y фиксирован, x пробегаем, далее следующий y.
    """
...
def _dense_mask_from_grid(y3_flat: np.ndarray, Hr: int, Wr: int, R: int, stride: int):
    """
    Делает плотную маску (Hr,Wr) из y3 (N,) для центров сетки.
    Способ: reshape -> resize nearest -> pad.
    """
    N = int(y3_flat.shape[0])
    ny, nx = _infer_grid_shape_from_centers(None, None, Hr, Wr, R, stride)
    if ny * nx != N:
        # fallback: делаем sparse по точкам (будет точечная)
        return None

    grid = y3_flat.reshape(ny, nx).astype(np.uint8)

    # upsample в область без рамки R (между центрами)
    out_h = Hr - 2*R
    out_w = Wr - 2*R
    up = cv2.resize(grid, (out_w, out_h), interpolation=cv2.INTER_NEAREST)

    # pad обратно до Hr x Wr
...
def _overlay_rgb(img_rgb: np.ndarray, mask_y3: np.ndarray, alpha=0.55):
    """
    mask_y3: (H,W) uint8 with 0/1/2
    2 fire -> red, 1 smoke -> white, 0 -> none
    """
...
def visualize_committee_masks_from_saved(
    h5_path: str,
    y3_pred_all,               # torch.Tensor or np.ndarray length N (samples)
    img_idx: int | None = None,
    max_tries: int = 50,
    alpha: float = 0.55,
):
    """
    Рисует: processed image (cropped+resized), GT mask, PRED mask, overlay.
    Берёт GT из h5 y_fire/y_smoke по тем же сэмплам.
    """
    ...
    with h5py.File(h5_path, "r") as h5:
        ...
        R_local = int(globals().get("R", 12))  # если R определён в ноуте; иначе 12 как типично для WINDOW=25
        ...
        # восстановить processed image (то, что реально использовалось для features)
        ...
        # отдельные бинарные пожар/не пожар
...
def colorize_mask(mask3):
    """
    mask3: (H,W) uint8 labels:
      0=non (black), 1=smoke (white), 2=fire (red), 3=sky (blue)
    returns RGB uint8
    """
    H,W = mask3.shape
    rgb = np.zeros((H,W,3), dtype=np.uint8)
    rgb[mask3==1] = (255,255,255)  # smoke
    rgb[mask3==2] = (255,0,0)      # fire
    rgb[mask3==3] = (0,0,255)      # sky
    return rgb
...
def visualize_committee_masks_one(
    ...
    with h5py.File(h5_path, "r") as h5:
        ...
        # pred 3-class on sampled points
        y3_pred = y3_pred_all[sl].cpu().numpy().astype(np.uint8)  # {0,1,2}

        # gt 3-class on sampled points (fire priority)
        y3_gt = np.zeros_like(y3_pred, dtype=np.uint8)
        y3_gt[y_smoke > 0] = 1
        y3_gt[y_fire  > 0] = 2

        # rebuild the cropped/resized image to match cy/cx coordinates
        ...
        # resize (same rule as in build)
        if was_resized == 1:
            img_c = cv2.resize(img_c, (rw, rh), interpolation=cv2.INTER_AREA)

        Hr, Wr = img_c.shape[:2]

        # rasterize masks on centers only
        ...
        # add "sky" to visualization (blue) as the cropped-out part, but only for display:
        # We'll show two versions:
        #  - cropped view (Hr x Wr) where sky doesn't exist
        #  - original view (H0 x W0) with sky region colored
        ...
        if was_resized == 0:
            ...
            # if resized, we still overlay in resized-cropped coordinates; for view:
            # show resized-cropped image itself to avoid geometry mismatch with original W0/H0
            img_show = img_c
            gt_full = gt_mask
            pr_full = pr_mask
